...
public class StickyPanel : CanvasDrawer
{
    ...
    [Header("Sticky Points")]
    [SerializeField] bool m_showStickyPoints = true;
    [SerializeField] int m_stickyPointCount = 20;
    ...
    private void OnEnable()
    {
        if (m_stickyPoints == null)
        {
            ...
            for (int i = 0; i < m_stickyPointCount; ++i)
            {
    ...
    protected override void Draw(CanvasGraphic canvas, Vector2 size)
    {
        ...
        if (m_showStickyPoints)
        {
            foreach (var p in m_stickyPoints)
            {
                Vector2 stickyPoint = p * innerSize - innerSize * 0.5f;
                canvas.CircleBrush.Draw(stickyPoint, 5f, layer: stickyLayer);
            }
        }
